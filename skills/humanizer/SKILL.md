...
description: |
  ...
  language, stock AI words, synonym cycling, false ranges, bold labels, or filler.
  Based on Wikipedia's "Signs of AI writing."
license: MIT
metadata:
  version: "3.0.0-local.1"
  upstream: https://github.com/blader/humanizer (v3.0.0)
...
**Problem:** The same thing is renamed each time it recurs, because a repetition penalty pushes the model toward a fresh synonym instead of the plain repeated noun. "The protagonist" becomes "the main character," then "the central figure," then "the hero"; in technical prose "the assay" becomes "the procedure," then "the technique." The reader has to check whether a new name means a new thing. Repeat the noun, or use a pronoun. *Weak alone.*
**Before:**
> The protagonist faces many challenges. The main character must overcome obstacles. The central figure eventually triumphs.
**After:**
> The protagonist faces many challenges but eventually overcomes them.

### 11b. False ranges

**Watch for:** from X to Y, spanning X to Y, ranging from X to Y
**Problem:** A "from X to Y" frame implies the two ends bound a scale and that the middle is covered, when the items are just examples on no common axis. Keep the range when X and Y are real endpoints of a measurable scale, such as a date, size, or price range. Otherwise list the items. *Weak alone.*
**Before:**
...
This copy is a local fork of [blader/humanizer](https://github.com/blader/humanizer) v3.0.0. Sections 11a (synonym cycling) and 11b (false ranges) are carried over from the earlier v2.7.0 of that skill, where they were sections 11 and 12; both recur in academic prose. Everything else matches upstream. When syncing a new upstream release, re-apply those two sections and the `allowed-tools` frontmatter.
