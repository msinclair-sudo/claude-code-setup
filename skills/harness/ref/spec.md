...
**An expansion is remembered by key, not by position.** The page polls every three seconds and rebuilds the panel, so anything opened closed itself a moment later — the information was right and unreadable. Open items are held in a set keyed on what the item *is*, and re-applied after every render; and a payload identical to the last one is not drawn at all, which also stops a redraw landing in the middle of a text selection.
...
**A block carries comments too, and the box is on the item itself.** The operator's answer to a question is the thing a stopped lane is waiting for, so it goes on the block record rather than into a message: the record outlives the session that raised it, and the next occupant of that node inherits the answer along with the question. The node is told at orientation (`answered N comment(s) on your open block(s)`), reads them with `harness blocked --list`, and the cursor advances for that reader only. A comment does **not** clear the block — granting does, or the node resolving it — because answering a question and widening a permission are different acts.

The box is rendered inside the expanded item rather than behind a click on some other surface. An affordance one click away on a page the reader is not looking at is an affordance that does not get found, which is what the first version was: one clickable chip on the whole page, and none of them on the four things actually waiting.
