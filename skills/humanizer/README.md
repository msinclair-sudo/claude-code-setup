...
> **Local fork.** This copy tracks [blader/humanizer](https://github.com/blader/humanizer)
> v3.0.0, with two additions: sections **11a (synonym cycling)** and **11b (false
> ranges)**, carried over from the earlier v2.7.0 where they were sections 11 and 12.
> Both recur in academic prose. The `allowed-tools` frontmatter is also restored.
> Deploy via `install.sh`, never by editing `~/.claude/skills/` directly.
> The previous version is kept as `SKILL.md.v2.7.0.bak`.
